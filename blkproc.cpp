...
int main(int argc, char *argv[])
{
  ...
  for (size_t c = 0; c < buf.size(); ++c)
  {
    ...
    blkproc(buf_f[c], block_dct2);      // 順方向
    // 量子化
    // ハフマン符号化
  }
  //----------DECODE
  for (size_t c = 0; c < buf.size(); ++c)
  {
    // ハフマン復号
    // 量子化
    blkproc(buf_f[c], block_idct2); // 逆方向
    buf[c] += 128;
...
void line_mozaic(cv::Mat &in) // 8×8のブロック内の値を左上の値を塗りつぶす
{
  if (in.rows != DCTSIZE || in.cols != DCTSIZE || in.channels() != 1)
  {
    printf("Input for mozaic() shall be 8*8 and monoclome\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (int y = 1; y < in.rows; ++y)
  {
    ...
    {
      in.data[y * DCTSIZE + x] = in.data[y * DCTSIZE + 0];
    }
  }
}
...
void block_dct2(cv::Mat &in)
{
  if (in.rows != DCTSIZE || in.cols != DCTSIZE || in.channels() != 1)
  {
    printf("Input for block_dct2() shall be 8*8 and monoclome\n");
    exit(EXIT_FAILURE);
  }
  cv::dct(in, in);
}
...
void block_idct2(cv::Mat &in)
{
  if (in.rows != DCTSIZE || in.cols != DCTSIZE || in.channels() != 1)
  {
    printf("Input for block_idct2() shall be 8*8 and monoclome\n");
    exit(EXIT_FAILURE);
  }
  cv::idct(in, in);
}